...
company_info = pd.read_csv(
    r"C:\Users\pwh94\OneDrive\Desktop\python\stock\t187ap03_L.csv",
    dtype={"公司代號": str}
)
...
def make_link(url, name):
    ...
    if not url.startswith("http://") and not url.startswith("https://"):
        url = "https://" + url
    # 移除前面的 localhost 或路徑，只保留正確網址
    url = url.split('/')[-1] if 'localhost' in url else url
...
styler = (
    ...
    .map(color_pos_neg, subset=['pct_change','rsi']+cols_inst+['三大法人加權合計'])
)
