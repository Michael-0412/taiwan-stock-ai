...
def calc_score(df_price, df_inst=None):
    df = df_price.copy()
    df['MA5'] = df['收盤價'].rolling(5, min_periods=1).mean()
    df['pct_chg'] = df['收盤價'].pct_change(periods=10).fillna(0) * 100
    df['vol_score'] = np.minimum(1.0, df['成交股數']/df['成交股數'].mean())
    df['ma_score'] = np.where(df['收盤價'] > df['MA5'],1.0,0.5)
    df['pct_score'] = np.where(df['pct_chg']>0,1.0,0.5)
    df['score'] = 0.4*df['vol_score'] + 0.3*df['ma_score'] + 0.2*df['pct_score']

    if df_inst is not None and not df_inst.empty:
        df = df.merge(df_inst, on='代號', how='left')
        df['inst_score'] = df['inst_score'].fillna(0.5)
        df['score'] += 0.1*df['inst_score']
    else:
        df['inst_score'] = 0.5  # 預設值
