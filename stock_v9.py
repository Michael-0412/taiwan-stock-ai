...
st.title("📈 台股 AI 選股器（收盤資料 + 三大法人統整）")
...
show_cols = ['名次','證券代號','證券名稱','收盤價','pct_change','成交金額','rsi'] + \
            cols_inst + ['三大法人加權合計','inst_score','score']

st.subheader("📊 前50名熱門股（收盤資料 + 三大法人）")
st.dataframe(
    df_top50[show_cols].style
        .format({
            "pct_change":"{:.2f}%",
            "rsi":"{:.1f}",
            "收盤價":"{:.2f}",
        ...
        .applymap(highlight, subset=['pct_change','rsi'] + cols_inst + ['三大法人加權合計']),
